	...
	// Log user in
		...
		try
		{
			...
			$sth = $db->prepare("SELECT user_id, username, users.active, users.first_name, users.middle_name, users.last_name, users.email,
										user_type, emp_id, emp_cat_id, dept_id, super_teacher, class_cat_limit
									FROM app.users
...
function sendSms($textBy, $text, $recipients, $client) {
	...
	// file_put_contents('smsLog.txt', print_r("(1) RECIPIENTS" . PHP_EOL, true), FILE_APPEND);
	// file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
	// file_put_contents('smsLog.txt', print_r(json_encode($recipients) . PHP_EOL, true), FILE_APPEND);
	$recipients = json_decode($recipients);
	$recipientsNum = count($recipients);
	file_put_contents('smsLog.txt', print_r("(1.1) RECIPIENTS COUNT" . PHP_EOL, true), FILE_APPEND);
	file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
	file_put_contents('smsLog.txt', print_r($recipientsNum . PHP_EOL, true), FILE_APPEND);
	$userPhone = $recipients[0]->phone_number;
	$userNm = $recipients[0]->recipient_name;

	// generate token & get client data
	$db = getSmsDB();
	$sth = $db->prepare("SELECT * FROM subscribers WHERE subscriber_name = :client");
	$sth->execute(array(':client' => $client));
	$results = $sth->fetch(PDO::FETCH_OBJ);
	file_put_contents('smsLog.txt', print_r("(2) CLIENT DATA" . PHP_EOL, true), FILE_APPEND);
	file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
	...
	if($results) {
	        ...
	        for ($i = 0; $i < $length; $i++) {
	            $randomString .= $include_chars [rand(0, $charLength - 1)];
					...
					$tme = date("h:i:sa");
					$datetime = $dte . " " . $tme;
					file_put_contents('smsLog.txt', print_r("(3) CORRELATOR" . PHP_EOL, true), FILE_APPEND);
					file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
	    ...
	    $data->correlator = generateRandomString($length);
			...
			if(count($recipients) == 1){
				file_put_contents('smsLog.txt', print_r("(4) JUST ONE RECIPIENT" . PHP_EOL, true), FILE_APPEND);
				file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
				file_put_contents('smsLog.txt', print_r($userPhone . PHP_EOL, true), FILE_APPEND);
				...
				for ($i=0; $i < count($recipients); $i++) {
					file_put_contents('smsLog.txt', print_r("(4) EACH RECIPIENT" . PHP_EOL, true), FILE_APPEND);
					file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
			...
			file_put_contents('smsLog.txt', print_r("(5) PAYLOAD" . PHP_EOL, true), FILE_APPEND);
			file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
			...
			try
	    {
	        ...
	        $msgId = $msgId->message_id;
					...
					file_put_contents('smsLog.txt', print_r("(6) MESSAGE ID" . PHP_EOL, true), FILE_APPEND);
					file_put_contents('smsLog.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
