...
$app->get('/getStudentCbcReportCards/:student_id', function ($studentId) {
    //Get student report cards
...
$app->get('/getStudentCbcReportCard/:student_id/:class_id/:term_id', function ($studentId, $classId, $termId) {
    //Get student report card
...
$app->get('/getLiveCbcReportCardData/:student_id/:class_id/:term_id', function ($studentId, $classId, $termId) {
    //Get student report card
    ...
    try
    {
        $db = getDB();
				...
				$sth = $db->prepare("SELECT d.*,
								...
								(
									SELECT report_data::json FROM app.report_cards_cbc rc WHERE student_id = :studentId AND class_id = :classId AND term_id = :termId
								) AS report_card_comments,
								(
													SELECT report_card_type FROM app.report_cards_cbc rc WHERE student_id = :studentId AND class_id = :classId AND term_id = :termId
												) AS report_card_type,
								(
								SELECT CASE
										WHEN report_card_type IN ('Playgroup','CBC')
										THEN (SELECT report_data FROM app.report_cards_cbc rc WHERE student_id = :studentId AND class_id = :classId AND term_id = :termId)
									END AS report_card_data
								FROM (
									SELECT report_card_type FROM app.report_cards_cbc rc WHERE student_id = :studentId AND class_id = :classId AND term_id = :termId
								)a
...
$app->post('/addCbcReportCard', function () use($app) {
  // Add report card
	...
	$studentId =		( isset($allPostVars['student_id']) ? $allPostVars['student_id']: null);
	$termId =			( isset($allPostVars['term_id']) ? $allPostVars['term_id']: null);
	$examTypeId =			( isset($allPostVars['exam_type_id']) ? $allPostVars['exam_type_id']: null);
	$classId =			( isset($allPostVars['class_id']) ? $allPostVars['class_id']: null);
	$reportCardType =	( isset($allPostVars['report_card_type']) ? $allPostVars['report_card_type']: null);
	$teacherId =		( isset($allPostVars['teacher_id']) ? $allPostVars['teacher_id']: null);
	$userId =			( isset($allPostVars['user_id']) ? $allPostVars['user_id']: null);
  ...
  try
  {
    $db = getDB();
		...
		$getReport = $db->prepare("SELECT cbc_report_card_id FROM app.report_cards_cbc WHERE student_id = :studentId AND class_id = :classId AND term_id = :termId AND exam_type_id = :examTypeId");

		$addReport = $db->prepare("INSERT INTO app.report_cards_cbc(student_id, class_id, term_id, exam_type_id, report_data, created_by, report_card_type, teacher_id, published)
								VALUES(:studentId, :classId, :termId, :examTypeId, :reportData, :userId, :reportCardType, :teacherId, :published)");
		$updateReport = $db->prepare("UPDATE app.report_cards_cbc
									SET report_data = :reportData,
											published = :published,
										modified_date = now(),
										modified_by = :userId
									WHERE cbc_report_card_id = :reportCardId");

		$db->beginTransaction();

		$getReport->execute( array(':studentId' => $studentId, ':classId' => $classId,':termId' => $termId, ':examTypeId' => $examTypeId ) );
		$reportCardId = $getReport->fetch(PDO::FETCH_OBJ);

		if( $reportCardId )
		{
			$updateReport->execute( array(':reportData' => $reportData, ':userId' => $userId, ':reportCardId' => $reportCardId->cbc_report_card_id, ':published' => $published ) );
		}
		else
		{
			$addReport->execute( array(':studentId' => $studentId,
