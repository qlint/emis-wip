...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
    <div class="container">
      <a class="navbar-brand" href="/" style="color:#0cff05;"><?php echo htmlspecialchars( array_shift((explode('.', $_SERVER['HTTP_HOST']))) ); ?>.eduweb.co.ke <span style="color:#ffffff;"> - Cancelled Invoices</span></a>
  ...
  <!-- End of Header Nav -->
  <div class="limiter">
    <h4 style="text-align:center;margin-top:85px;">This page lists all cancelled invoices, along with the student, date, invoice amount and wo cancelled it.</h4>
    ...
    <div class="container-table100">
         ...
         <h4>Cancelled Invoices:: <?php echo $term_name; ?></h4><hr>
<?php

   echo "<h4>Student invoice balances and credits report for ". $term_name ." (Ordered by Names Ascending)</h4><hr>";
   $table3 = pg_query($db,"SELECT i.inv_id, s.admission_number, s.first_name || ' ' || coalesce(s.middle_name,'') || ' ' || s.last_name AS student_name,
                          	TO_CHAR(i.inv_date, 'DD-MON-YYYY') AS inv_date, total_amount, TO_CHAR(i.due_date, 'DD-MON-YYYY') AS due_date,
                          	TO_CHAR(i.creation_date, 'DD-MON-YYYY') AS created_on,
                          	u1.first_name || ' ' || coalesce(u1.middle_name,'') || ' ' || u1.last_name AS created_by,
                          	u2.first_name || ' ' || coalesce(u2.middle_name,'') || ' ' || u2.last_name AS canceled_by,
                          	term_name
                          FROM app.invoices i
                          INNER JOIN app.students s USING (student_id)
                          LEFT JOIN app.users u1 ON i.created_by = u1.user_id
                          LEFT JOIN app.users u2 ON i.modified_by = u2.user_id
                          INNER JOIN app.terms t USING (term_id)
                          WHERE i.canceled IS TRUE AND i.term_id = $term
                          ORDER BY student_name ASC, inv_id ASC");
        ...
        echo "<div id='t3' class='table100-head'>";
           echo "<thead>";
            echo "<tr class='row100 head'>";
              echo "<th class='cell100 column10'>INV #</th>";
              echo "<th class='cell100 column10'>ADM #</th>";
              echo "<th class='cell100 column1'>STUDENT NAME</th>";
              echo "<th class='cell100 column1'>INV DATE</th>";
              echo "<th class='cell100 column10'>TOT AMT.</th>";
              echo "<th class='cell100 column9'>DUE DATE</th>";
              echo "<th class='cell100 column1'>CREATED</th>";
              echo "<th class='cell100 column1'>CREATED BY</th>";
              echo "<th class='cell100 column1'>CANCELED BY</th>";
              echo "<th class='cell100 column12'>TERM</th>";
            echo "</tr>";
           echo "</thead>";
       echo "</div>";
         ...
         echo "<tbody>";
           while ($row3 = pg_fetch_assoc($table3)) {
             ...
             echo "<tr class='row100 body'>";
                echo "<td class='cell100 column10'>" . $row3['inv_id'] . "</td>";
                echo "<td class='cell100 column10'>" . $row3['admission_number'] . "</td>";
                echo "<td class='cell100 column1'>" . $row3['student_name'] . "</td>";
                echo "<td class='cell100 column1'>" . $row3['inv_date'] . "</td>";
                echo "<td class='cell100 column10'>" . number_format($row3['total_amount']) . "</td>";
                echo "<td class='cell100 column9'>" . $row3['due_date'] . "</td>";
                echo "<td class='cell100 column1'>" . $row3['created_on'] . "</td>";
                echo "<td class='cell100 column1'>" . $row3['created_by'] . "</td>";
                echo "<td class='cell100 column1'>" . $row3['canceled_by'] . "</td>";
                echo "<td class='cell100 column12'>" . $row3['term_name'] . "</td>";
            echo "</tr>";
           }
         echo "</tbody>";
       echo "</div>";
