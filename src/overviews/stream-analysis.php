  ...
  <div class="limiter">
    ...
    <div style="border: 3px dashed #397C49; width:20%; margin-left:auto;margin-right:auto;text-align:center;padding-bottom:7px;padding-top:7px;">
      ...
      <form style="margin-left:auto; margin-right:auto; text-align:center;" action="#" method="post">
        ...
        <div><input style="margin-left:auto; margin-right:auto; text-align:center;" type="submit" name="submit" value="Get Results For This Term" onclick="getStreamTitle();" /></div>
      </form>
    </div>
    <?php
    ...
    <div class="container-table100" style="align-items: flex-start;">
  	    <div class="wrap-table100">
            <h4 id="expTitle" style="border-left:7px solid;border-color:#3DE100;background-color:#D8FFC9;">
                <?php $title = 'ajax/docTitles/' . array_shift((explode('.', $_SERVER['HTTP_HOST']))) . '_' . $class . '_' . $term . '_streamDocTitle.txt'; echo readfile($title); ?>
            </h4>
            <hr>
            ...
            $examResults = pg_query($db,"SELECT app.colpivot('_exam_marks', 'SELECT gender, first_name || '' '' || coalesce(middle_name,'''') || '' '' || last_name as student_name,
                                                                    ...
                                                                    WHERE class_subjects.class_id IN (SELECT class_id FROM app.classes WHERE class_cat_id IN (SELECT class_cat_id FROM app.class_cats WHERE entity_id = $class)) 
                                                                    AND term_id = $term
                            ...
                            <tr class='row100 head' id="tblHeader">
                                ...
                                <?php
                                    ...
                                    foreach ($header as $column => $value) {
                                        ...
                                        if(strpos($subjectName, 'student_name') !== false){$subjectName = "STUDENT";}
                                        if(strpos($subjectName, 'rank') !== false){$subjectName = "POS";}
                                        if(strpos($subjectName, 'gender') !== false){$subjectName = "GND";}
                                        if(strpos($subjectName, 'Math') !== false){$subjectName = "MAT";}
                                        if(strpos($subjectName, 'Engl') !== false){$subjectName = "ENG";}
                                        if(strpos($subjectName, '&') !== false){$subjectName = "SS/RE";}
                                        if(strpos($subjectName, 'Social Studies') !== false){$subjectName = "SS";}
                                        if(strpos($subjectName, 'Kisw') !== false){$subjectName = "KIS";}
                                        if(strpos($subjectName, 'Scie') !== false){$subjectName = "SCI";}
                                        if(strpos($subjectName, 'CRE') !== false){$subjectName = "CRE";}
                                        if(strpos($subjectName, 'Lang') !== false){$subjectName = "E.Lng";}
                                        if(strpos($subjectName, 'Compo') !== false){$subjectName = "E.Cmp";}
                                        if(strpos($subjectName, 'Lugh') !== false){$subjectName = "K.Lgh";}
                                        if(strpos($subjectName, 'Insh') !== false){$subjectName = "K.Ins";}
                                        /*
