                                            ...
                                            FROM   crosstab('SELECT class_name, grade, grade_count FROM (
                                              								SELECT class_name, grade, count(grade) as grade_count FROM (
                                              									SELECT student_name, class_name, round((total_mark::float/800)*100) as percentage, (select grade from app.grading where round((total_mark::float/800)*100) between min_mark and max_mark) as grade FROM (
                                              											SELECT student_name, class_name, class_id, sum(total_mark) as total_mark, sum(total_grade_weight) as total_grade_weight FROM (
                                              												SELECT  student_name, class_name, class_id, subject_name,
                                              												    round(avg(total_mark)) as total_mark, round(avg(total_grade_weight)) as total_grade_weight, sort_order
                                              												FROM (
                                              												 ...
                                              												 ) q GROUP BY student_name, class_name, class_id, subject_name, sort_order
                                              												    ORDER BY sort_order
                                              											)v GROUP BY student_name, class_name, class_id ORDER BY student_name ASC, class_name DESC
