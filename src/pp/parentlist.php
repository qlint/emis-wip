...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark bg-dark fixed-top">
    <div class="container">
      <a class="navbar-brand" href="/" style="color:#0cff05;"><?php echo htmlspecialchars( array_shift((explode('.', $_SERVER['HTTP_HOST']))) ); ?>.eduweb.co.ke <span style="color:#ffffff;"> - Guardians</span></a>
  ...
  <!-- End of Header Nav -->
  <div class="limiter">
    <h4 style="text-align:center;margin-top:85px;">Guardians, active students and their classes.</h4>
    <!--
    <div style="border: 3px dashed #397C49; width:20%; margin-left:auto;margin-right:auto;text-align:center;padding-bottom:7px;padding-top:7px;">
      ...
      <form style="margin-left:auto; margin-right:auto; text-align:center;" action="#" method="post">
        <select name="Term">
          <option value='3'>Term 1</option>
          <option value='5'>Term 2</option>
          <option value='4'>Term 3</option>
        </select>
    ...
    $class_name = "";
    if($term == 3){$term_name = "Term 1";}elseif($term == 5){$term_name = "Term 2";}elseif($term == 4){$term_name = "Term 3";}
    */
    ?>
    <div class="container-table100">
  	   <div class="wrap-table100">
         <!-- <p style="text-align:center;">Currently showing results for <?php echo $term_name; ?></p> -->
         <h4>List of all guardians in the school and their students</h4><hr>
<?php
...
$table1 = pg_query($db,"SELECT g.first_name || ' ' || coalesce(g.middle_name,'') || ' ' || g.last_name AS parent_name, 
	g.telephone, s.first_name || ' ' || coalesce(s.middle_name,'') || ' ' || s.last_name AS student_name,
	c.class_name
...
WHERE s.active IS TRUE AND g.active IS TRUE
ORDER BY c.class_id ASC, parent_name ASC");
    ...
    // echo "<table id='table1'>";
      echo "<thead>";
       echo "<tr class='row100 head'>";
         echo "<th class='cell100 column1'>PARENT</th>";
         echo "<th class='cell100 column3'>TELEPHONE</th>";
         echo "<th class='cell100 column4'>STUDENT</th>";
         echo "<th class='cell100 column5'>CLASS</th>";
       echo "</tr>";
      echo "</thead>";
    // echo "</table>";
  echo "</div>";
      ...
      echo "<tbody>";
        while ($row = pg_fetch_assoc($table1)) {
          echo "<tr class='row100 body'>";
             ...
             echo "<td class='cell100 column3'>" . $row['telephone'] . "</td>";
             echo "<td class='cell100 column4'>" . $row['student_name'] . "</td>";
             echo "<td class='cell100 column5'>" . $row['class_name'] . "</td>";
         echo "</tr>";
        }
      echo "</tbody>";
    // echo "</table>";
    ...
    $(document).ready(function() {
      $('#table1').DataTable( {
          ...
          buttons: [
            ...
            {
              extend: 'pdfHtml5',
              title: window.location.host.split('.')[0] + '-Guardian-List'
            }
          ],
          "order": [[ 0, "asc" ]]
      } );
