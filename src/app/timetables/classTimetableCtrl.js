...
controller('classTimetableCtrl', ['$scope', '$rootScope', 'apiService','$timeout','$window','$q','$parse',
function($scope, $rootScope, apiService, $timeout, $window, $q, $parse){
	...
	$scope.setUpTt = function(){
        ...
        $scope.loadSubjects();
        $scope.timetableTitle = $("#classSelect option:selected").text() + ' Timetable For ' + $("#termSelect option:selected").text();;
        ttView.style.display = "block";
        // get the class timetable
        $scope.fetchClassTimetable($scope.selectedClassTt.class_id, $scope.selectedClassTt.term_id);
    }
    
    $scope.fetchClassTimetable = function(classId, termId){
        console.log("class_id = " + classId + " and term_id = " + termId);
        let timetableParam = classId + '/' + termId;
        apiService.fetchClassTimetable(timetableParam, function(response,status){
			var result = angular.fromJson(response);
				
			if( result.response == 'success')
			{	
				$scope.classTimetable = ( result.nodata ? [] : result.data );
				if($scope.classTimetable.length > 0){ initialize(); }
                function initialize()
                {
                    console.log($scope.classTimetable);
                    for (let a = 0; a < $scope.classTimetable.length; a++) { 
                        var startHour = $scope.classTimetable[a].start_hour; 
                        var startMinutes = $scope.classTimetable[a].start_minutes;
                        var endHour = $scope.classTimetable[a].end_hour; 
                        var endMinutes = $scope.classTimetable[a].end_minutes;
                        var subjectName = $scope.classTimetable[a].subject_name;
                        var day = $scope.classTimetable[a].day;
                        var color = $scope.classTimetable[a].color;
                        var options = {url: '#', class: color};
                        timetable.addEvent(subjectName, day, new Date(2020,7,17,startHour,startMinutes), new Date(2020,7,17,endHour,endMinutes),{url: '#', class: color});
