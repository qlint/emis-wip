...
function($scope, $rootScope, apiService, $timeout, $window, $filter, $state){
	...
	$scope.gridOptions = {
		...
		rowHeight:24,
		columnDefs: [
			{ name: 'Date', field: 'creation_date', type:'date', enableColumnMenu: false, sort: {direction:'desc'}, cellTemplate:'<div class="ui-grid-cell-contents" ng-click="grid.appScope.viewEmail(row.entity)">{{row.entity.creation_date|date:"MMM d yyyy, h:mm a"}}</div>'},
