...
function($scope, $rootScope, apiService, $dialogs, FileUploader, $timeout, $state){
	...
	*/
	...
	$scope.$watch('filters.com_type',function(newVal, oldVal){
		...
		$scope.theemployee.selected = undefined;
	...
	$scope.$watch('filters.audience',function(newVal, oldVal){
		...
		// if employee, show employee select
		$scope.theemployee2.selected = undefined;
		$scope.isEmployee = ( newVal.audience == 'Employee' ? true : false );
		if( $scope.isEmployee )
		{
	...
	var loadEmployeesForComms = function(response)
	{
		...
		if( result.response == 'success')
		{
			$scope.employeesForComms = ( result.nodata ? {} : result.data );
            ...
            for(z = 0; z < $scope.employeesForComms.length; z++){
                var employeeObj = {
                    employee_full_name: $scope.employeesForComms[z].employee_name,
                    department: $scope.employeesForComms[z].dept_name,
                    category: $scope.employeesForComms[z].emp_cat_name,
                    telephone: $scope.employeesForComms[z].telephone,
                    emp_id: $scope.employeesForComms[z].emp_id
                }
                $scope.employeeComms.push(employeeObj);
                console.log($scope.employeeComms);
            }
			if( $scope.post.post_id !== null && $scope.post.post_id !== undefined )
			{
				/* set emp */
				$scope.theemployee2.selected = $scope.employees.filter(function(item){
