...
controller('homeworkFeedbackCtrl', ['$scope', '$rootScope', 'apiService','$timeout','$window','$state',
function($scope, $rootScope, apiService, $timeout, $window, $state){
	...
	var rowTemplate = function()
	{
		...
		'  <div ng-if="!row.entity.merge" ng-repeat="(colRenderIndex, col) in colContainer.renderedColumns track by col.colDef.name" class="ui-grid-cell" ng-class="{ \'ui-grid-row-header-cell\': col.isRowHeader }" ng-click="grid.appScope.viewHomeworkFeedback(row)" data-target="#privilegesModal"  ui-grid-cell></div>' +
		'</div>';
	}
	...
	$scope.gridOptions = {
		...
		exporterCsvFilename: 'homework_feedback.csv',
	...
	var getHomeworkFeedback = function()
	{
		if( $rootScope.currentUser.user_type == 'TEACHER' ){ var params = "/" + $rootScope.currentUser.emp_id; }
		else{ params += "/0"; }
		apiService.getHomeworkFeedback(params, function(response,status){
	...
	$scope.singleFilter = function( renderableRows )
	{
		...
		renderableRows.forEach( function( row ) {
		  var match = false;
		  [ 'class_name', 'subject_name', 'parent_name', 'student_name', 'message', 'title' ].forEach(function( field ){
	...
	$scope.viewHomeworkFeedback = function(row)
	{
