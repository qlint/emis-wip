...
function($scope,$rootScope,$uibModalInstance,apiService,data){
		...
		if( $scope.guardian.first_name == null || $scope.guardian.first_name == undefined || $scope.guardian.last_name == null || $scope.guardian.last_name == undefined ){
		    ...
		    setTimeout(function(){ 
		        ...
		        var testSelect = $("#idbtn").css("display", "none");
					...
					'<div class="col-sm-6">' +
						...
						'<div class="form-group" ng-class="{ \'has-error\' : (parentForm.$submitted || parentForm.id_number.$dirty ) && parentForm.id_number.$invalid && parentForm.id_number.$error.required }">' +
							...
							'<div class="col-sm-4 nopad-right">' +
								...
								'<div ng-show="edit||add" class="row">' +
    									'<input id="id_number" type="text" name="id_number" ng-model="guardian.id_number" ng-model-options="{ debounce: 1000 }" class="form-control col-sm-6" required numeric-only pattern="[0-9]{8,} />' +
    									'<!--<span class="col-sm-4" id="idbtn" ng-show="isNewParent">' + 
    									    '<button ng-click="genRandId()" type="button" class="btn btn-xs btn-primary">Generate Random ID</button>' + 
    									'</span>-->' + 
