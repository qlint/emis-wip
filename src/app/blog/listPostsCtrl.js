...
function($scope, $rootScope, apiService, $timeout, $window, $filter, $state){
	...
	{		
		if( $rootScope.postTypes === undefined )
		{
			apiService.getBlogPostTypes({}, function(response){
				var result = angular.fromJson(response);
				
				// store these as they do not change often
				if( result.response == 'success') 
				{
					$scope.postTypes = result.data;
					$rootScope.postTypes = $scope.postTypes;
				}		
		...
		{
			$scope.postTypes = $rootScope.postTypes;
		}
	...
	{
		$scope.openModal('blog', 'previewPost', 'md', post);
	}
	
	$scope.addPost = function()
	{		
		$state.go('add_post', {class_id: $scope.filters.class_id});
	}
	...
	$scope.viewPost = function(item)
	{
		...
		$state.go('edit_post', {post: item, post_id: item.post_id, class_id: selectedClass.class_id, selectedClass: selectedClass});
	}
