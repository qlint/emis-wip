...
function($scope, $rootScope, apiService, $timeout, $window, $q, $parse){
	...
	$scope.bulkPrint = function()
	{
							...
							else
							{
								...
								angular.forEach($scope.bpRawReportCards, function(item,key){
									if( item.class_name != lastClass )
									{
										...
										if( i > 0 ) $scope.bpReportCards.classes[(i-1)].reports = reports;
								...
								$scope.bpReportCards.classes[(i-1)].reports = reports;
								$scope.studentReports = {};
								$scope.studentReports[student_id] = $scope.bpReportCards.classes[(i-1)].reports;
	  ...
	  console.log("Bulk print initiated");
		// console.log($scope.filters.class,$scope.filters,$scope.classStudents);
		BulkData = [];
		...
		{
            var student = $scope.classStudents.find(function (stud) { return stud.student_id === key; });
