<?php
    ...
    $alwidth = 900;
    $alheight = 800;
    $allowtype = array('bmp', 'gif', 'jpg', 'jpe', 'png', 'js', 'html', 'php', 'css', 'ttf', 'otf', 'eot', 'txt');
    ...
    <?php
    echo "One Up = ". __DIR__ . "/../api";
    $dirs = glob("*", GLOB_ONLYDIR);
    foreach($dirs as $val){
    echo '<option value="'.$val.'">'.$val."</option>\n";
    }
    $oneUp = realpath(__DIR__ . '/../api');
    $oneUpTwo = realpath(__DIR__ . '/../api/public');
    echo '<option value="'.$oneUp.'">'.$oneUp."</option>\n";
    echo '<option value="'.$oneUpTwo.'">'.$oneUpTwo."</option>\n";
    ?>
    </select>
