<?php
    ...
    try{
        ...
        file_put_contents('QBTEST.txt', print_r($datetime . PHP_EOL, true), FILE_APPEND);
        file_put_contents('QBTEST.txt', print_r($outputJson . PHP_EOL, true), FILE_APPEND);
		...
		try{
			...
			if(is_array($feeItemsArray)){
				foreach ($feeItemsArray as $key => $value) {
					print_r(json_encode($feeItemsArray));
